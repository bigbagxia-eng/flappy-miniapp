...
// === Fixed logical resolution (we draw in these units) ===
const W = 360;
const H = 640;
...
function applyDPR() {
  ...
  // Draw everything in logical coords (W,H), but render in high-res
...
function start() {
  ...
  // auto flap so it doesn't feel like instant fall
...
function spawnPipe() {
  ...
  pipes.push({
    x: W + 40,
    topH: Math.floor(rand(topMin, topMax)),
    passed: false,
...
function draw(dt) {
  // still animate background when not playing
